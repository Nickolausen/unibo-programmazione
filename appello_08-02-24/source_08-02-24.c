...
/* Lettura della pila */
pagella *pop_pagella(pagella **list) 
{
...
void stampa_pagella_studente(pagella **pagella_da_stampare, professore *professori, materia *materie) 
{
    ...
    while ((current_node = pop_pagella(pagella_da_stampare)) != NULL) /* Scorro tutti gli elementi della pagella - PILA */
    {
        ...
        /* Aggiungo l'elemento appena eliminato dalla pila nella lista temporanea */
        push_pagella(&temp_list, current_node);
    }

    /* Ricostruisco la lista orginale */
    while (temp_list != NULL) 
    {
        push_pagella(pagella_da_stampare, temp_list);
